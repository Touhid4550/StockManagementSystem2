...
namespace StockManagementSystem
{
    public partial class StockInUi : Form
    {
        ...
        {
            items.CompanyId = Convert.ToInt32(companyComboBox.SelectedValue);
            ...
            try
            {
                int availableQuantity = Add(items.Id, stockInQuantity);
                bool isUpdate = UpdateAvailableQuantity(items.Id, availableQuantity);
                if (!isUpdate)
                {
                ...
                // Save Data
                Connect();
                sqlCommand.CommandText = @"INSERT INTO StockIn(StockInQuantity,Date,CompanyID,ItemID) VALUES('" + stockInQuantity + "','" + nowDateTime.ToShortDateString() + "','" + items.CompanyId + "','" + items.Id + "')";
        ...
        private DataTable GetItemCombo(Items items)
        {
            Connect();
            sqlCommand.CommandText = @"SELECT ID ,Name FROM Items where CompanyId=" + items.CompanyId + "";                        
           // DatabaseConnection.sqlConnection.Open();
            sqlDataAdapter = new SqlDataAdapter(sqlCommand);
            dataTable = new DataTable();
            sqlDataAdapter.Fill(dataTable);
        ...
        {
            items.CompanyId = Convert.ToInt32(companyComboBox.SelectedValue);
            itemComboBox.DataSource = GetItemCombo(items);
